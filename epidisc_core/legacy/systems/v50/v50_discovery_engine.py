...
class V50DiscoveryEngine:
    ...
    def _init_modules(self):
        ...
        if self.config.enable_simulation:
            try:
                from epidisc_core.reasoning.v50_world_simulator import (
                    WorldModelInterface, create_world_simulator
                )
                self.world_simulator = create_world_simulator()
            except ImportError:
                try:
                    from epidisc_core.capabilities.v50_world_simulator import (
                        WorldModelInterface, create_world_simulator
                    )
                    self.world_simulator = create_world_simulator()
                except ImportError:
        ...
        if self.config.enable_program_synthesis:
            try:
                from epidisc_core.reasoning.v50_program_synthesis import (
                    ProgramSynthesizer, create_program_synthesizer
                )
                self.program_synthesizer = create_program_synthesizer()
            except ImportError:
                try:
                    from epidisc_core.capabilities.v50_program_synthesis import (
                        ProgramSynthesizer, create_program_synthesizer
                    )
                    self.program_synthesizer = create_program_synthesizer()
                except ImportError:
        ...
        if self.config.enable_causal_reasoning:
            try:
                from epidisc_core.reasoning.v50_causal_engine import (
                    CausalInferenceEngine, create_causal_engine
                )
                self.causal_engine = create_causal_engine()
            except ImportError:
                try:
                    from epidisc_core.capabilities.v50_causal_engine import (
                        CausalInferenceEngine, create_causal_engine
                    )
                    self.causal_engine = create_causal_engine()
                except ImportError:
        ...
        if self.config.enable_meta_learning:
            try:
                from epidisc_core.reasoning.v50_meta_learner import (
                    MetaLearningSystem, create_meta_learner
                )
                self.meta_learner = create_meta_learner()
            except ImportError:
                try:
                    from epidisc_core.capabilities.v50_meta_learner import (
                        MetaLearningSystem, create_meta_learner
                    )
                    self.meta_learner = create_meta_learner()
                except ImportError:
        ...
        if self.config.enable_debate:
            try:
                from epidisc_core.reasoning.v50_adversarial_debate import (
                ...
                try:
                    from epidisc_core.capabilities.v50_adversarial_debate import (
                        AdversarialDebateReasoner, create_debate_reasoner
                    )
                    self.debate_system = create_debate_reasoner(
                        max_rounds=self.config.debate_rounds
        ...
        if self.config.enable_abstraction:
            try:
                from epidisc_core.reasoning.v50_abstraction_learning import (
                    HierarchicalAbstractionLearner, create_abstraction_learner
                )
                self.abstraction_learner = create_abstraction_learner()
            except ImportError:
                try:
                    from epidisc_core.capabilities.v50_abstraction_learning import (
                        HierarchicalAbstractionLearner, create_abstraction_learner
                    )
                    self.abstraction_learner = create_abstraction_learner()
                except ImportError:
        ...
        if self.config.use_v43_base:
            try:
                from epidisc_core.core.v43 import create_v43_gpqa
